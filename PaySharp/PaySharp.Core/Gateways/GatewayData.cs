...
namespace PaySharp.Core
{
    ...
    public class GatewayData
    {
            ...
            void Add(MemberInfo[] info)
            {
                foreach (var item in info)
                {
                    ...
                    else
                    {
                        ...
                        {
                            key = StringUtil.ToCamelCase(item.Name);
                        }
            ...
            try
            {
                ...
                foreach (Match item in mc)
                {
                    ...
                    if (isUrlDecode)
                    {
                        ...
                        //支持二次解码
                        if (value.IsMatch("%[0-9a-z]{2}", RegexOptions.IgnoreCase))
                            value = value.UrlDecode();
                    }
        ...
        public void FromForm(IFormCollection form)
        {
            ...
            {
                var value = form[item].ToString();
                if (!string.Equals("sign", item, StringComparison.CurrentCultureIgnoreCase))
                    value = value.UrlDecode();
                Add(item, value);
            }
        }

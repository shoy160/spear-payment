...
namespace Acb.Gateway.Payment.ViewModels
{
    ...
    public class VPaymentSignature : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            AcbHttpContext.Current.SetProject(project);
            if (string.IsNullOrWhiteSpace(project.Secret) || Consts.Mode == ProductMode.Dev)
                return results;
            if (Sign.Length != 45)
            {
            ...
            var logger = LogManager.Logger<VPaymentSignature>();
            logger.Debug(JsonHelper.ToJson(dict));

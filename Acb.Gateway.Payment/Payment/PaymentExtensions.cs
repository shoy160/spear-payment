...
namespace Acb.Gateway.Payment.Payment
{
    internal static class PaymentExtensions
    {
        ...
        private static readonly ILogger Logger = LogManager.Logger(typeof(PaymentExtensions));
                ...
                case PaymentMode.Wechat:
                    switch (type)
                    {
                        ...
                        case PaymentType.H5:
                            ...
                            var clientIp = AcbHttpContext.Current.RemoteIp();
                            Logger.Info($"[h5]client-ip:{clientIp}");
                            ...
                            if (!string.IsNullOrWhiteSpace(trade.RedirectUrl))
                                url = "redirect_url".SetQuery(trade.RedirectUrl, url);
                            return url;
                        case PaymentType.Scan:
